...
async def create_copilot_session(session_id: str, model: str, config: dict) -> dict:
    ...
    skill_url = "https://raw.githubusercontent.com/dermatologist/crisp-t/refs/heads/develop/.agents/skills/crisp-cli/SKILL.md"
    try:
        response = requests.get(skill_url)
        response.raise_for_status()
        skill_content = response.text
    except Exception as e:
        skill_content = "CRISP-T skill instructions could not be loaded. Please check the online resource."
    system_message = {
        "role": "system",
