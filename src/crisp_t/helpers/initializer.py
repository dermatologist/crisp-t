...
def initialize_corpus(
    ...
    if inp:
        ...
        corpus = read_data.read_corpus_from_json(
            inp, comma_separated_ignore_words=comma_separated_ignore_words if comma_separated_ignore_words else ""
        )
        if corpus:
            click.echo(f"✓ Successfully loaded {len(corpus.documents)} document(s) from {inp}")
            return corpus
        else:
